...
_X, _y, seed = read_csv(file_path, scaler, "we")

_X, eval_X, _y, eval_y = train_test_split(_X, _y, random_state=42, shuffle=True, test_size=0.1)

pred_bayes, error, mse_bayes, mape_bayes = eval_bayesian_rbf(np.copy(_X), np.copy(_y), np.copy(eval_X),
                                                             np.copy(eval_y),0.1)

fig = plt.figure()
ax = fig.add_subplot(111, projection='3d')

ax.scatter(eval_X.T[0], eval_X.T[2], eval_y, color='blue', label="true", alpha=0.7, lw=2, marker="o", )
ax.scatter(_X.T[0], _X.T[2], _y, c='g', label="train_y", alpha=0.7, marker="*", lw=1)
ax.scatter(eval_X.T[0], eval_X.T[2], pred_bayes, label='bayesian', c='red', alpha=0.7, marker="X", lw=3)

for i in range(len(error)):
    ax.plot([eval_X.T[0][i], eval_X.T[0][i]], [eval_X.T[2][i], eval_X.T[2][i]],
            [pred_bayes[i] - error[i] * 1000, pred_bayes[i] + error[i] * 1000], c='red', alpha=0.7)

ax.set_ylabel("Workload Mix", labelpad=23)
ax.set_xlabel("Concurrent Users", labelpad=23)
# ax.set_zlabel('Average Latency (ms)', labelpad=23)
ax.set_zlabel('Throughput', labelpad=23)
